...
| `RequeueExpiredLeases` | Sweeps the Task Fabric for expired leases and returns the requeued task IDs (failure path 1). |
...
- `aresrecovery` -> `internal/fabric/task` (via `tasks *taskfabric.Fabric`; import path `internal/fabric/task`): sweeps expired leases via `CheckExpiredLeases`, acquires tasks for replacement agents, reads preserved checkpoints. `RequeueExpiredLeases` returns the requeued task IDs (`[]string`).
- `aresrecovery` -> `internal/fabric/agent` (via `agents *agentfabric.Fabric`; import path `internal/fabric/agent`): spawns replacement agents, installs cognitive state via `SetCognitiveState` and `Recover`.
...
- `aresrecovery` -> `internal/kernel` (System Runtime control plane): the Recovery subsystem is adopted as a component (`Orchestrator.Adopt`) and stopped by the Orchestrator; wiring in `cmd/ares/kernel.go` and `internal/ares_bootstrap/system_runtime_wiring.go`.
...
## Strategy score write-back (zero-LLM feedback loop)

`ScoredFeedbackAdapter.Apply` computes an aggregate score via
`DeterministicScorer` from execution attribution and writes it to the active
strategy through `StrategyScoreAdapter.WriteActiveScore` → StrategyStore.
The write-back requires an active strategy — bootstrap seeds `bootstrap-root`
into an empty store; without that seed every write fails with "no active
strategy" and the GA accumulates no fitness. The adapter never changes which
strategy is active: it updates the Score field in place.
